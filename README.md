...
recordings, dictation, PDFs — indexes it on your own machine, and lets an AI
...
**MCP.** The live server is `src-tauri/src/mcp_api.rs` — MCP over SSE
implemented directly in Rust on `127.0.0.1:47847`, no Node required. It exposes
four tools: `check_connection`, `list_assistants`, `list_tags` and `search`,
the last going through the same retrieval path as the in-app assistant. Point
any MCP client at it:

```json
{ "mcpServers": { "kety-knowledge": { "url": "http://127.0.0.1:47847/sse" } } }
```

Note that the MCP server has **no authentication**: it is bound to loopback, but
any process running as you can query your knowledge base through it while the
app is open.
...
- **`mcp-server/` is dead.** The Node package proxies to `/mcp/*` routes the
  Rust server no longer serves, so three of its four tools would 404. Its
  header still tells you to wire it into Claude Code. Don't; use the URL above.
